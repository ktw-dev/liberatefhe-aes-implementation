...
def sub_word(flat_key: np.ndarray, max_blocks: int = 2048) -> np.ndarray:
    """Apply SubWord operation to the last word (bytes 12-15) of flat key array.
    
    Applies S-box substitution to each byte in the last 4-byte word.
    Uses S-box indexing for substitution with masking for position selection.
    ...
    Returns
    -------
    substituted : np.ndarray, same shape as flat_key
        Key array with SubWord (S-box) applied to last word (positions 12-15)
    """
    result = flat_key.copy()
    total_len = 16 * max_blocks
    positions = np.arange(total_len)
    
    # Create mask for the last word (bytes 12-15)
    last_word_mask = (positions >= 12 * max_blocks) * (positions < 16 * max_blocks)
    
    # Extract values from last word using masking
    last_word_values = flat_key * last_word_mask
    
    # Apply S-box substitution using indexing (exception allowed for S-box)
    substituted_values = AES_SBOX[last_word_values] * last_word_mask
    
    # Combine: keep original values for positions outside last word, use substituted for last word
    result = flat_key * (1 - last_word_mask) + substituted_values
