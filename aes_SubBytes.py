...
def _get_coeff_plaintexts(engine_context: CKKS_EngineContext, which: str):
    """Return dict[(p,q)] → encoded-coeff plaintexts for given engine & "hi"/"lo"."""
    key = (id(engine_context.get_engine()), which)
    if key in _PT_CACHE:
    ...
    for (p, q), c in coeffs.items():
        ...
        pt_dict[(p, q)] = engine_context.get_engine().encode(vec)
...
def _poly_eval(engine_context: CKKS_EngineContext, ct_hi: Any, ct_lo: Any, which: str):
    basis_x = _build_power_basis(engine_context, ct_hi)
    basis_y = _build_power_basis(engine_context, ct_lo)

    coeff_pt = _get_coeff_plaintexts(engine_context, which)

    # start with zero ciphertext (scale/level match)
    cipher_res = engine_context.ckks_multiply(ct_hi, 0.0)

    for (p, q), pt in coeff_pt.items():
        term = engine_context.ckks_multiply(basis_x[p], basis_y[q])
        term = engine_context.ckks_multiply(term, pt)
        cipher_res = engine_context.ckks_add(cipher_res, term)

    return cipher_res

def _ones_cipher(engine_context: CKKS_EngineContext):
    """Encrypt vector of all-ones directly (avoids scale mismatch)."""
    ones_vec = np.ones(engine_context.get_slot_count(), dtype=np.complex128)
    return engine_context.ckks_encrypt(ones_vec)

def _build_power_basis(engine_context: CKKS_EngineContext, ct):
    ...
    basis[0] = _ones_cipher(engine_context)
